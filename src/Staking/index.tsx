...
const Staking = () => {
    ...
    const handleStaking = async () => {
        try {
            ...
            if (MaxTHCTotalAmountToWallet === 0) return tips("Please enter token amount for staking");
            if (MaxTHCTotalAmountToWallet < 30000) return tips("Can't staking less than 30,000")
            if (MaxTHCTotalAmountToWallet > 15000000) return tips("Can't staking more than 15,000,000")

            if (status === "notConnected") return tips("Please connect Metamask wallet")
            // if (loading) return tips("in progress")
            // setLoading(true);

            // THC token check on users wallet
            if (THCTotalAmountToWallet <= 0) return tips("You have not any THC Token.");
    ...
    const stakingMax = async () => {
        setMaxTHCTotalAmountToWallet(THCTotalAmountToWallet);
    }
            ...
            <div className="container">
                <div className='stake-panel'>
                    <div className='panel-title'>
                        <h3>Staking for Transhuman Coin</h3>
                    </div>
